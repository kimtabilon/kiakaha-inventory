...
class TransactionController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($payment['name']=='Add Item to Inventory') {
            foreach ($inventories as $inventory) {
                ...
                if ($match) {
                    ...
                    if ($images          ->count()) {
                        $new_inv->itemImages()          ->attach($images        ->last());
                    }
                    if ($refImages       ->count()) {
                        $new_inv->itemRefImages()       ->attach($refImages     ->last());
                    }
                    if ($prices          ->count()) {
                        $new_inv->itemPrices()          ->attach($prices        ->last());
                    }
                    if ($sellingPrices   ->count()) {
                        $new_inv->itemSellingPrices()   ->attach($sellingPrices ->last());
                    }
                    if ($restorePrices   ->count()) {
                        $new_inv->itemRestorePrices()   ->attach($restorePrices ->last());
                    }
                }
