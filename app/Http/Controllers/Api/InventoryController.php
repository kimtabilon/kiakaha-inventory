        ...
        if ($left > 0) {
            ...
            if ($discounts   ->count()) {
                foreach ($discounts as $v) {
                    $inventory->itemDiscounts()    ->attach($v);
                }
            }
            if ($transactions->count()) {
                foreach ($transactions as $v) {
                    $inventory->transactions()     ->attach($v);
                }
            }
            if ($donors      ->count()) {
                foreach ($donors as $v) {
                    $inventory->donors()           ->attach($v);
                }
            }
            
            if ($codes           ->count()) {
                $inventory->itemCodes()         ->attach($codes         ->last());
            }
            if ($images          ->count()) {
                $inventory->itemImages()        ->attach($images        ->last());
            }
            if ($refImages       ->count()) {
                $inventory->itemRefImages()     ->attach($refImages     ->last());
            }
            if ($prices          ->count()) {
                $inventory->itemPrices()        ->attach($prices        ->last());
            }
            if ($sellingPrices   ->count()) {
                $inventory->itemSellingPrices() ->attach($sellingPrices ->last());
            }
            if ($restorePrices   ->count()) {
                $inventory->itemRestorePrices() ->attach($restorePrices ->last());
            }
        } else {
            $left=0;
            $inventory                  = Inventory::find($data['inventory']['id']);
